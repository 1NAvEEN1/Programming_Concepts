...
Naveen Jayathilake IM/2019/044
...
int main(){
	while(1){
		...
		char op = getche();//get a direct input from the keyboard to select the relevent option.(No need to press enter affter inputing a number like in getchar().)
		...
		if(op=='1'){
			...
			instructions();//to go to the instructin menue
		}else if(op=='3'){Beep(200,500);
...
//function which is used to draw the dsisks of the tower
void drawDisks(int tower, int tileNo, int y){
	int x;
	if( tower == 1 ) x = 35;
